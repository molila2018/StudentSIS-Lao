...
Last updated: 2026-07-14 (post-data-layer-refactor + views-split)
...
| | |
|---|---|
| **UI** | Imperative C# code-behind — one file per page in `Views/` (`H.MkGrid`, `H.Btn` builders live in `Views/UiHelpers.cs`). A few pages use XAML (`ReportPage`, `ScoresPage`, `StudentsPage`, `LoginWindow`, `StudentFormWindow`). |
| **Data layer** | `static partial class DB` split across domain files in `Data/` (`DB.cs` core + `DB.ScoreEntry.cs`, `DB.Students.cs`, `DB.Dashboard.cs`, `DB.ClassHub.cs`, `DB.Enrollment.cs`, `DB.Promotion.cs`, `DB.Admin.cs`, `DB.Reports.cs`). **ALL SQL lives in the Data layer as named methods — view files must never contain SQL strings.** No ORM, no Repository pattern — still direct ADO.NET behind a static facade. |
...
| **Formatting** | Compact / one-liner methods are common in the older builder-style pages (UsersPage, SettingsPage); the newer code (ReportPage, UserFormWin) is more spacious. |
...
│
├─ Data\
│  ├─ DB.cs                               (core: connection, schema+migrations, settings
│  │                                       cache, calc helpers, cross-page history queries)
│  ├─ DB.ScoreEntry.cs                    (score-entry rosters + saves — pages idx 5, 6)
│  ├─ DB.Students.cs                      (student CRUD/search + StudentDto + CSV import row)
...
│  ├─ DB.Admin.cs                         (Subjects · Users · Years · log — idx 9-12; YearStats)
│  ├─ DB.Reports.cs                       (report-builder queries — per-student + class bulk)
│  └─ ExcelImport.cs                      (Excel template gen + parse/validate/UPSERT
│                                          for both score-entry pages; shared by tests)
│
├─ Views\                                 (ONE FILE PER PAGE — RemainingPages.cs was split 2026-07)
│  ├─ MainWindow.xaml + .cs               (sidebar + page host)
│  ├─ LoginWindow.xaml + .cs              (empty fields, no hints)
│  ├─ DashboardPage.xaml + .cs            (idx 0)
│  ├─ ClassHubPage.cs                     (idx 1)
│  ├─ StudentsPage.xaml + .cs             (idx 2) + StudentFormWindow.xaml + .cs
│  ├─ EnrollmentPage.cs                   (idx 3)
│  ├─ BatchEnrollPage.cs                  (idx 4)
│  ├─ ScoresPage.xaml + .cs               (idx 5)
│  ├─ MonthlyScoresPage.cs                (idx 6 — + MonthlyRow + MonthItem models)
│  ├─ ReportPage.xaml + .cs               (idx 7 — report builders + xlsx/docx/PDF helpers)
│  ├─ PromotionPage.cs                    (idx 8 — + PromRow + PromotionConfirmWindow)
│  ├─ SubjectsPage.cs                     (idx 9 — + SubjectFormWin)
│  ├─ UsersPage.cs                        (idx 10 — + UserFormWin)
│  ├─ AcademicYearPage.cs                 (idx 11 — + Delete/Stats windows + AcademicYearFormWin)
│  ├─ SettingsPage.cs                     (idx 12 — General / CSV / Backup / Log tabs)
│  ├─ ScoreHistoryPage.cs                 (idx 13 — roster picker)
│  ├─ StudentHistoryWindow.cs             (📖 per-student viewer + HistoryReportCatalog)
│  ├─ ClassHistoryWindow.cs               (📚 class-wide viewer)
│  ├─ ImportPreviewWindow.cs              (Excel-import preview dialog)
│  ├─ SaveConfirmDialog.cs                (3-button dirty-state prompt + SaveConfirmResult)
│  ├─ ComboItems.cs                       (ComboItem + SubComboItem shared combo models)
│  └─ UiHelpers.cs                        (internal static class H — grid/card/button builders)
│
├─ Helpers\
...
│
└─ tests\
   └─ IntegrationTests\                   (console app, net8.0 — links Data\*.cs via wildcard)
...
`Views/RemainingPages.cs` no longer exists — it was split into the one-file-per-page layout above (pure line moves, no logic changes). When adding a new page, create its own file in `Views/`.
...
1. **Match the surrounding style.** Older builder-style pages (UsersPage, SettingsPage) use dense one-liners; ReportPage and newer code uses spacious multi-line — don't mix the two within a method.
...
10. **No SQL in view files.** Every query/write goes through a named method on the `DB` partial class — add it to the matching `Data/DB.*.cs` domain file (see §12 layout). Views read like prose: `DB.GetSemesterScoreRoster(...)`, never `DB.Query("SELECT ...")`.
...
- New page in sidebar → `MainWindow.xaml.cs` (`_pages` array) + a NEW file `Views\<PageName>.cs`
- New query/write → named method in the matching `Data\DB.*.cs` domain file (never SQL in the view)
- New report → `ReportPage.xaml` (combobox item) + `ReportPage.xaml.cs` (`BtnPdf_Click` case + new `GenX` method)
